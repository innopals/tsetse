...
function thenableText(nodeType: string, isVariable: boolean) {
  return (
    `Found a thenable ${isVariable ? 'variable' : 'return value'} being` +
...
function checkConditional(checker: Checker, node: ts.ConditionalExpression) {
  addFailureIfThenableCallExpression(
    checker,
    node.condition,
    thenableReturnText('a conditional'),
  );

  addFailureIfThenableIdentifier(
    checker,
    node.condition,
    thenableVariableText('a conditional'),
  );
}
...
function checkBinaryExpression(checker: Checker, node: ts.BinaryExpression) {
  ...
  addFailureIfThenableCallExpression(
    checker,
    node.left,
    thenableReturnText('a binary expression'),
  );

  addFailureIfThenableIdentifier(
    checker,
    node.left,
    thenableVariableText('a binary expression'),
  );
}
...
function checkWhileStatement(checker: Checker, node: ts.WhileStatement) {
  addFailureIfThenableCallExpression(
    checker,
    node.expression,
    thenableReturnText('a while statement'),
  );

  addFailureIfThenableIdentifier(
    checker,
    node.expression,
    thenableVariableText('a while  statement'),
  );
}
...
function checkIfStatement(checker: Checker, node: ts.IfStatement) {
  addFailureIfThenableCallExpression(
    checker,
    node.expression,
    thenableReturnText('an if statement'),
  );

  addFailureIfThenableIdentifier(
    checker,
    node.expression,
    thenableVariableText('an if statement'),
  );
}
