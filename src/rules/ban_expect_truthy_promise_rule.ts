...
function checkForTruthy(checker: Checker, node: ts.PropertyAccessExpression) {
  ...
  if (
    !ts.isIdentifier(expectCallNode.expression) ||
    expectCallNode.expression.text !== 'expect'
  ) {
    return;
  }

  if (
    expectCallNode.arguments.length === 0 ||
    ts.isAwaitExpression(expectCallNode.arguments[0])
  ) {
    return;
  }
