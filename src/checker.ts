...
export class Checker {
  ...
  execute(sourceFile: ts.SourceFile): Failure[] {
    const run = (node: ts.Node) => {
      // Dispatch handlers registered via `on`
      this.dispatchNodeHandlers(node);

      // Dispatch handlers for named identifiers and properties
      if (ts.isIdentifier(node)) {
        this.dispatchNamedIdentifierHandlers(node);
      } else if (ts.isPropertyAccessExpression(node)) {
        this.dispatchNamedPropertyAccessHandlers(node);
      } else if (ts.isElementAccessExpression(node)) {
        this.dispatchStringLiteralElementAccessHandlers(node);
      }
